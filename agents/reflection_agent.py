...
class Memory:
    ...
    def add_record(self, record_type: str, content: str):
        """
        像记忆中加如一条记录
        :param record_type: 记录的类型 如 execution 或 reflection
        :param content: 记录的具体内容 如 生成的代码 或 反思的反馈
        :return:
        """
    ...
    def get_trajectory(self) -> str:
        ...
        for record in self.records:
            if record['type'] == 'execution':
                ...
                trajectory_parts.append(f"--- 评论员反馈 ---\n{record['content']}")

        return  "\n\n".join(trajectory_parts)

    def get_last_execution(self) -> Optional[str]:
        """
        获取最近一次执行结果，例如 最后一次生成的代码
        如果不存在，返回None
        :return:
        """
